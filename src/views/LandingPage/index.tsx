import koi1200x1600 from "./koi1200x1600.jpg";
import koi3000x4000 from "./koi3000x4000.jpg";
import koi300x400 from "./koi300x400.jpg";
import koi600x800 from "./koi600x800.jpg";
import orca1200x1600 from "./orca1200x1600.jpg";
import orca3000x4000 from "./orca3000x4000.jpg";
import orca300x400 from "./orca300x400.jpg";
import orca600x800 from "./orca600x800.jpg";
import { Transition } from "@headlessui/react";
import { clsx } from "clsx/lite";

export const LandingPage = () => (
  <Transition
    appear={true}
    enter="transition-opacity ease-in"
    enterFrom="opacity-75"
    enterTo="opacity-100"
    show={true}
  >
    <div className="min-w-sm min-h-screen bg-gray-50">
      <div className="m-auto min-h-screen max-w-5xl border border-gray-100 bg-white px-4 pb-4 pt-2 shadow shadow-gray-50 sm:px-16 sm:pb-16 sm:pt-8">
        <nav className="m-auto h-12 w-fit space-x-4 rounded-full border border-gray-200 px-5 shadow shadow-gray-200">
          <a
            className={clsx(
              "inline-flex h-full items-center px-1 font-bold text-gray-600",
              // Active
              "border-b-2 border-black text-black",
              // Focus
              "focus-visible:border-2 focus-visible:border-blue-500 focus-visible:text-blue-500 focus-visible:outline-none",
              // Hover
              "hover:border-b-4 hover:border-black hover:text-black",
            )}
            href="/"
          >
            About
          </a>
          <a
            className={clsx(
              "inline-flex h-full items-center px-1 font-bold text-gray-600",
              // Focus
              "focus-visible:border-2 focus-visible:border-blue-500 focus-visible:text-blue-500 focus-visible:outline-none",
              // Hover
              "hover:border-b-4 hover:border-black hover:text-black",
            )}
            href="https://github.com/kamadorueda"
            rel="noopener noreferrer"
            target="_blank"
          >
            Projects
          </a>
          <a
            className={clsx(
              "inline-flex h-full items-center px-1 font-bold text-gray-600",
              // Focus
              "focus-visible:border-2 focus-visible:border-blue-500 focus-visible:text-blue-500 focus-visible:outline-none",
              // Hover
              "hover:border-b-4 hover:border-black hover:text-black",
            )}
            href="https://linkedin.com/in/kamadorueda"
            rel="noopener noreferrer"
            target="_blank"
          >
            Resume
          </a>
        </nav>

        <main className="mt-8 sm:mt-16">
          <img
            alt="Kevin's profile"
            className={clsx(
              "h-16 w-16 rounded-full",
              // Hover
              "hover:scale-125 hover:fill-black hover:transition-all",
            )}
            src="https://avatars.githubusercontent.com/u/47480384?v=4"
          />

          <h1 className="mt-8 text-4xl font-bold text-black">
            Software Engineer and hobbyist Painter
          </h1>

          <p className="mt-8 text-justify text-xl text-black">
            I'm Kevin, a Software Engineer based in Calgary, Canada who paints
            in his free time.
          </p>

          <div className="mt-8 space-x-8">
            <a
              className="group inline-block focus-visible:outline-none"
              href="mailto:kamadorueda@gmail.com"
            >
              <svg
                className={clsx(
                  "h-8 w-8 fill-gray-500 p-1",
                  // Hover
                  "group-hover:scale-125 group-hover:fill-black group-hover:transition-all",
                  // Focus
                  "group-focus-visible:border-2 group-focus-visible:border-blue-500 group-focus-visible:fill-blue-500 group-focus-visible:outline-none",
                )}
                role="img"
              >
                <title>Kevin's Email</title>
                <use xlinkHref="#gmail"></use>
              </svg>
            </a>
            <a
              className="group inline-block focus-visible:outline-none"
              href="https://github.com/kamadorueda"
            >
              <svg
                className={clsx(
                  "h-8 w-8 fill-gray-500 p-1",
                  // Hover
                  "group-hover:scale-125 group-hover:fill-black group-hover:transition-all",
                  // Focus
                  "group-focus-visible:border-2 group-focus-visible:border-blue-500 group-focus-visible:fill-blue-500 group-focus-visible:outline-none",
                )}
                role="img"
              >
                <title>Kevin's GitHub profile</title>
                <use xlinkHref="#github"></use>
              </svg>
            </a>
            <a
              className="group inline-block focus-visible:outline-none"
              href="https://linkedin.com/in/kamadorueda"
            >
              <svg
                className={clsx(
                  "h-8 w-8 fill-gray-500 p-1",
                  // Hover
                  "group-hover:scale-125 group-hover:fill-black group-hover:transition-all",
                  // Focus
                  "group-focus-visible:border-2 group-focus-visible:border-blue-500 group-focus-visible:fill-blue-500 group-focus-visible:outline-none",
                )}
                role="img"
              >
                <title>Kevin's LinkedIn profile</title>
                <use xlinkHref="#linkedin"></use>
              </svg>
            </a>
          </div>

          <div className="my-16 overflow-x-auto outline">
            <div className="m-auto w-max space-x-16 p-4 outline">
              <a
                className={clsx(
                  "group inline-block focus-visible:outline-none",
                )}
                href={koi3000x4000}
              >
                <picture>
                  <source media="(max-width: 300px)" srcSet={koi300x400} />
                  <source media="(max-width: 600px)" srcSet={koi600x800} />
                  <source media="(max-width: 1200px)" srcSet={koi1200x1600} />
                  <img
                    className={clsx(
                      "object-scale-up h-96 w-72 rounded-2xl shadow grayscale transition",
                      "-rotate-2",
                      // Hover
                      "group-hover:grayscale-0",
                      // Focus
                      "group-focus-visible:border-2 group-focus-visible:border-blue-500 group-focus-visible:fill-blue-500 group-focus-visible:p-1 group-focus-visible:outline-none group-focus-visible:grayscale-0",
                    )}
                    alt="A koi fish Kevin painted"
                    src={koi3000x4000}
                  />
                </picture>
              </a>
              <a
                className={clsx(
                  "group inline-block focus-visible:outline-none",
                )}
                href={orca3000x4000}
              >
                <picture>
                  <source media="(max-width: 300px)" srcSet={orca300x400} />
                  <source media="(max-width: 600px)" srcSet={orca600x800} />
                  <source media="(max-width: 1200px)" srcSet={orca1200x1600} />
                  <img
                    className={clsx(
                      "object-scale-up h-96 w-72 rounded-2xl shadow grayscale transition",
                      "rotate-2",
                      // Hover
                      "group-hover:grayscale-0",
                      // Focus
                      "group-focus-visible:border-2 group-focus-visible:border-blue-500 group-focus-visible:fill-blue-500 group-focus-visible:p-1 group-focus-visible:outline-none group-focus-visible:grayscale-0",
                    )}
                    alt="An orca Kevin painted"
                    src={orca3000x4000}
                  />
                </picture>
              </a>
            </div>
          </div>
        </main>
      </div>

      {/* SVG Icons from https://simpleicons.org/ */}
      <svg className="hidden" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <symbol id="github" viewBox="0 0 24 24">
            <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
          </symbol>
          <symbol id="gmail" viewBox="0 0 24 24">
            <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.69 2.28 24 3.434 24 5.457z" />
          </symbol>
          <symbol id="linkedin" viewBox="0 0 24 24">
            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
          </symbol>
        </defs>
      </svg>
    </div>
  </Transition>
);
